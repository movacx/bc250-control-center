...
class BaseOSRepository:
    ...
    def prepare_dependencies_command(
        self, component: str = "all", *, cu_manager_script: str = "", mode: str = "apply"
    ) -> str:
        if mode not in {"apply", "check", "plan"}:
            raise ValueError(f"Unsupported dependency operation mode: {mode}")
        env = {
        ...
        return self._script_command(
            self.dependency_script, "--component", component, "--mode", mode, env=env
        )
