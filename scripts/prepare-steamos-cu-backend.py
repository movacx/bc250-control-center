...
a GC register namespace by itself does not bind that static ASIC model; UMR
therefore cannot find the BC-250-only WGP register definitions unless the
static cyan_skillfish model and the database-defined GC block are used together.
...
DEFAULT_ASIC_REPLACEMENT = 'ASIC="${UMR_ASIC:-}"'


def _replace_default_asic(source: str) -> str:
    """Use the GC namespace declared by cyan_skillfish.asic.

    The physical ASIC is commonly described as gfx1013, but the register-block
    label is database data. Leave the selector empty at startup so the
    validated SteamOS database can supply the active GC namespace.
    """
...
ENSURE_DATABASE_REPLACEMENT = r'''bc250_resolve_asic_from_database() {
  local ip_common
  if [ -n "$ASIC" ]; then
    export UMR_ASIC="$ASIC"
    return 0
  fi
  ip_common="$(awk 'NR > 1 && NF == 4 && ($1 ~ /^gfx/ || $2 == "GC") { print $1; exit }' "$UMR_DATABASE_PATH/cyan_skillfish.asic" 2>/dev/null || true)"
  [ -n "$ip_common" ] || die "CU_UMR_SELECTOR_UNKNOWN: cyan_skillfish.asic does not declare a GC register block"
  ASIC="cyan_skillfish.$ip_common"
  export UMR_ASIC="$ASIC"
}
...
ensure_umr_database() {
  ...
  if [ -n "$UMR_DATABASE_PATH" ] && umr_database_looks_valid "$UMR_DATABASE_PATH"; then
    bc250_resolve_asic_from_database
    return 0
  fi
