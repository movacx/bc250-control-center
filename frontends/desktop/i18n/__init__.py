...
from .dialog_catalog import DIALOG_TRANSLATIONS
from .interface_catalog import INTERFACE_TRANSLATIONS
from .locale_catalog import COMPLETE_LOCALES, load_locale_catalog, locale_fallback_chain
from .memory_catalog import MEMORY_TRANSLATIONS
from .polish_catalog import polish_translation
from .system_setup_catalog import SYSTEM_SETUP_TRANSLATIONS
...
assert tuple(code for code, _name in LANGUAGE_OPTIONS if code != "auto") == COMPLETE_LOCALES
_CURRENT_LANGUAGE = "en"


_GOVERNOR_FIX_LABELS = {
    "metrics": {
        "en": "Fix metrics", "es": "Corregir métricas", "es-419": "Corregir métricas", "pt": "Corrigir métricas", "pt-BR": "Corrigir métricas",
        "ru": "Исправить метрики", "uk": "Виправити метрики", "de": "Metriken korrigieren",
        "pl": "Napraw metryki", "fr": "Corriger les métriques", "zh-CN": "修正指标",
        "zh-TW": "修正指標", "ja": "メトリクスを修正", "ko": "메트릭 수정",
        "it": "Correggi metriche", "cs": "Opravit metriky", "tr": "Metrikleri düzelt",
        "nl": "Metrieken corrigeren", "ro": "Corectează metricile", "hu": "Mérőszámok javítása",
        "bg": "Поправи метриките", "da": "Ret målinger", "fi": "Korjaa mittarit",
        "el": "Διόρθωση μετρήσεων", "id": "Perbaiki metrik", "ms": "Baiki metrik",
        "no": "Rett målinger", "sv": "Korrigera mätvärden", "th": "แก้ไขเมตริก",
        "vi": "Sửa chỉ số",
    },
    "frequency": {
        "en": "Fix frequencies", "es": "Corregir frecuencias", "es-419": "Corregir frecuencias", "pt": "Corrigir frequências", "pt-BR": "Corrigir frequências",
        "ru": "Исправить частоты", "uk": "Виправити частоти", "de": "Frequenzen korrigieren",
        "pl": "Napraw częstotliwości", "fr": "Corriger les fréquences", "zh-CN": "修正频率",
        "zh-TW": "修正頻率", "ja": "周波数を修正", "ko": "주파수 수정",
        "it": "Correggi frequenze", "cs": "Opravit frekvence", "tr": "Frekansları düzelt",
        "nl": "Frequenties corrigeren", "ro": "Corectează frecvențele", "hu": "Frekvenciák javítása",
        "bg": "Поправи честотите", "da": "Ret frekvenser", "fi": "Korjaa taajuudet",
        "el": "Διόρθωση συχνοτήτων", "id": "Perbaiki frekuensi", "ms": "Baiki frekuensi",
        "no": "Rett frekvenser", "sv": "Korrigera frekvenser", "th": "แก้ไขความถี่",
        "vi": "Sửa tần số",
    },
}

_FIRMWARE_PERSISTENCE_LABELS = {
    "en": "Firmware persistence", "es": "Persistencia por firmware", "es-419": "Persistencia por firmware", "pt": "Persistência por firmware", "pt-BR": "Persistência por firmware",
    "ru": "Постоянство через прошивку", "uk": "Збереження через прошивку", "de": "Firmware-Persistenz",
    "pl": "Trwałość przez firmware", "fr": "Persistance du micrologiciel", "zh-CN": "固件持久化",
    "zh-TW": "韌體持久化", "ja": "ファームウェア永続化", "ko": "펌웨어 영속성", "it": "Persistenza firmware",
    "cs": "Trvalost firmwaru", "tr": "Ürün yazılımı kalıcılığı", "nl": "Firmwarepersistentie",
    "ro": "Persistență prin firmware", "hu": "Firmwarees megőrzés", "bg": "Постоянство чрез фърмуер",
    "da": "Firmware-persistens", "fi": "Laiteohjelmiston pysyvyys", "el": "Μόνιμη ρύθμιση υλικολογισμικού",
    "id": "Persistensi firmware", "ms": "Kekekalan perisian tegar", "no": "Fastvarepersistens",
    "sv": "Firmwarebeständighet", "th": "การคงค่าในเฟิร์มแวร์", "vi": "Lưu bền bằng phần sụn",
}


def governor_fix_label(kind: str, language: str | None = None) -> str:
    """Return the concise, reviewed label for a Cyan telemetry correction."""
    lang = resolve_language(language or _CURRENT_LANGUAGE)
    return _GOVERNOR_FIX_LABELS.get(str(kind), {}).get(lang, _GOVERNOR_FIX_LABELS["metrics"]["en"])


def firmware_persistence_label(language: str | None = None) -> str:
    lang = resolve_language(language or _CURRENT_LANGUAGE)
    return _FIRMWARE_PERSISTENCE_LABELS.get(lang, _FIRMWARE_PERSISTENCE_LABELS["en"])


def normalize_language(value: object) -> str:
    raw = str(value or "auto").strip().split(".", 1)[0].replace("_", "-").lower()
    aliases = {
        "automatic": "auto", "automático": "auto", "automatico": "auto", "system": "auto",
        "english": "en", "español": "es", "spanish": "es", "português": "pt", "portuguese": "pt",
        "latam": "es-419", "spanish-latam": "es-419", "spanish-latin-america": "es-419",
        "brazilian": "pt-BR", "brazilian portuguese": "pt-BR", "portuguese-brazil": "pt-BR",
        "русский": "ru", "russian": "ru", "українська": "uk", "ukrainian": "uk",
        "deutsch": "de", "german": "de", "polski": "pl", "polish": "pl", "french": "fr",
        "schinese": "zh-CN", "tchinese": "zh-TW", "japanese": "ja", "koreana": "ko",
        "italian": "it", "czech": "cs", "turkish": "tr", "dutch": "nl", "romanian": "ro",
        "hungarian": "hu", "bulgarian": "bg", "danish": "da", "finnish": "fi", "greek": "el",
        "indonesian": "id", "malay": "ms", "norwegian": "no", "swedish": "sv", "thai": "th",
        "vietnamese": "vi",
    }
    canonical = {code.lower(): code for code in COMPLETE_LOCALES}
    normalized = aliases.get(raw, canonical.get(raw))
    if normalized is None and "-" in raw:
        normalized = aliases.get(raw.split("-", 1)[0], canonical.get(raw.split("-", 1)[0]))
    return normalized if normalized in SUPPORTED_LANGUAGES or normalized == "auto" else "auto"
...
def _entry(en: str, es: str, pt: str, ru: str, uk: str, de: str) -> tuple[str, dict[str, str]]:
    return en, {
        "en": en, "es": es, "pt": pt, "ru": ru, "uk": uk, "de": de,
        "pl": polish_translation(en),
    }
...
_EXACT = dict([
    ...
    _entry(
        "CPU OC is unavailable on Alpine: bc250_smu_oc requires the exact stress binary, while Alpine packages stress-ng with an incompatible interface.",
        "El OC de CPU no está disponible en Alpine: bc250_smu_oc requiere el binario exacto stress, mientras Alpine ofrece stress-ng con una interfaz incompatible.",
        "O OC de CPU não está disponível no Alpine: bc250_smu_oc requer o binário exato stress, enquanto o Alpine fornece stress-ng com uma interface incompatível.",
        "Разгон CPU недоступен в Alpine: bc250_smu_oc требует точный бинарный файл stress, а Alpine предоставляет stress-ng с несовместимым интерфейсом.",
        "Розгін CPU недоступний в Alpine: bc250_smu_oc потребує точний двійковий файл stress, а Alpine надає stress-ng з несумісним інтерфейсом.",
        "CPU-OC ist unter Alpine nicht verfügbar: bc250_smu_oc benötigt genau das Programm stress, Alpine liefert jedoch stress-ng mit einer inkompatiblen Schnittstelle.",
    ),
    _entry(
        "The installed Compute Units service points to a manager that no longer exists. The operation was stopped before changing the hardware.",
        "El servicio instalado de Unidades de Cómputo apunta a un administrador que ya no existe. La operación se detuvo antes de modificar el hardware.",
        "O serviço instalado de Unidades de Computação aponta para um gerenciador que não existe mais. A operação foi interrompida antes de alterar o hardware.",
        "Установленная служба вычислительных блоков ссылается на отсутствующий менеджер. Операция остановлена до изменения оборудования.",
        "Встановлена служба обчислювальних блоків посилається на відсутній менеджер. Операцію зупинено до зміни обладнання.",
        "Der installierte Compute-Units-Dienst verweist auf einen nicht mehr vorhandenen Manager. Der Vorgang wurde vor einer Hardwareänderung angehalten.",
    ),
    _entry(
        "A stale Compute Units boot service points to a missing manager. Copy the recovery command, run it in a terminal, then reboot. You can also use Restore factory: it removes the stale service and restores the 24-CU factory topology.",
        "Un servicio de arranque obsoleto de Unidades de Cómputo apunta a un administrador inexistente. Copia el comando de recuperación, ejecútalo en una terminal y reinicia. También puedes usar Restaurar de fábrica: elimina el servicio obsoleto y restaura la topología de fábrica de 24 CU.",
        "Um serviço de inicialização obsoleto de Unidades de Computação aponta para um gerenciador ausente. Copie o comando de recuperação, execute-o em um terminal e reinicie. Você também pode usar Restaurar fábrica: ele remove o serviço obsoleto e restaura a topologia de fábrica de 24 CUs.",
        "Устаревшая служба загрузки вычислительных блоков ссылается на отсутствующий менеджер. Скопируйте команду восстановления, выполните её в терминале и перезагрузитесь. Также можно выбрать Восстановить заводские: это удалит устаревшую службу и восстановит заводскую топологию 24 CU.",
        "Застаріла служба завантаження обчислювальних блоків посилається на відсутній менеджер. Скопіюйте команду відновлення, виконайте її в терміналі та перезавантажтесь. Також можна вибрати Відновити заводські: це видалить застарілу службу і відновить заводську топологію 24 CU.",
        "Ein veralteter Compute-Units-Startdienst verweist auf einen fehlenden Manager. Kopieren Sie den Wiederherstellungsbefehl, führen Sie ihn im Terminal aus und starten Sie neu. Alternativ entfernt Werkseinstellungen wiederherstellen den veralteten Dienst und stellt die werkseitige 24-CU-Topologie wieder her.",
    ),
    ...
    _entry("The optional daemon requires an active systemd user manager and is unavailable on OpenRC.", "El daemon opcional requiere un gestor de usuario systemd activo y no está disponible con OpenRC.", "O daemon opcional requer um gerenciador de usuário systemd ativo e não está disponível no OpenRC.", "Дополнительному демону требуется активный пользовательский менеджер systemd; в OpenRC он недоступен.", "Необов’язковому демону потрібен активний менеджер користувача systemd; у OpenRC він недоступний.", "Der optionale Daemon benötigt einen aktiven systemd-Benutzermanager und ist unter OpenRC nicht verfügbar."),
...
_EXACT.update(MEMORY_TRANSLATIONS)
_EXACT.update(SYSTEM_SETUP_TRANSLATIONS)
_EXACT.update(CPU_TRANSLATIONS)
_EXACT.update(DIALOG_TRANSLATIONS)

@lru_cache(maxsize=len(COMPLETE_LOCALES))
def _reverse_locale_catalog(language: str) -> dict[str, str]:
    """Build reverse indexes lazily so startup does not parse every locale."""
    reverse: dict[str, str] = {}
    for source, translated in load_locale_catalog(language).items():
...
def _canonical_source_cached(source: str, lang: str) -> str:
    if lang == "en" or not source:
        return source
    return _reverse_locale_catalog(lang).get(source, source)
...
PROJECT_OVERVIEW["pl"] = """BC250 Control Center to graficzny interfejs do zarządzania, przygotowywania i monitorowania narzędzi społeczności dla płyty AMD BC-250.

Możliwości aplikacji:
• Łączy w jednym oknie obsługę GPU, CPU, 40CU, procesów, pamięci, czujników i historii.
• Przygotowuje zależności w obsługiwanych dystrybucjach, aby świeża instalacja była gotowa bez wyszukiwania poleceń w internecie.
• Udostępnia profile i działania wraz z ostrzeżeniami, walidacją i widocznymi limitami.
• Pokazuje ścieżki, stan usług i ważne komunikaty, dzięki czemu użytkownik wie, co się dzieje.

Używane narzędzia:
• cyan-skillfish-governor-smu do sterowania zakresami GPU przez interfejs governor/D-Bus.
• bc250_smu_oc do tymczasowego lub trwałego OC CPU, gdy użytkownik wybierze tę opcję.
• bc250-cu-live-manager do podglądu na żywo i działań 40CU/24CU.
• nct6687d do eksperymentalnego sterowania PWM wentylatorów po przygotowaniu modułu.
• UMR, lm_sensors, systemd, Polkit i narzędzia systemowe, gdy są wymagane.

Autorzy i zakres projektu:
• Naszą pracą jest graficzny launcher i centrum sterowania, które integruje, porządkuje i ułatwia obsługę tych repozytoriów.
• Nie przypisujemy sobie autorstwa zewnętrznych narzędzi społeczności; każde zachowuje własnych autorów, licencję i oficjalne repozytorium.
• Sterownik nct6687d zachowuje autorstwo i licencję projektu Fred78290/nct6687d.
• Celem jest wygodna obsługa BC-250 bez ciągłego korzystania z terminala i przechodzenia między wieloma repozytoriami.

Ważne:
• Overclocking, 40CU i zmiany częstotliwości mogą spowodować zawieszenie, wyłączenie, utratę danych lub uszkodzenie sprzętu.
• Każda BC-250 jest inna; testuj stopniowo i na własną odpowiedzialność."""

DAEMON_DETAILS["pl"] = """Opcjonalny demon użytkownika bc250-control-centerd monitoruje temperaturę, RAM, swap i stan governora oraz zapisuje metryki JSONL nawet po zamknięciu GUI.
...
def _short_count_form(count: int, unit: str, language: str) -> str:
    return _localized_count_label(count, unit, language)


@lru_cache(maxsize=32768)
def _tr_cached(source: str, lang: str) -> str:
    # Check the requested/regional catalog first, but do not let the English
    # disk fallback mask a newer reviewed exact translation.  This ordering is
    # important while catalogs are augmented between releases: otherwise a
    # newly added Spanish/German/etc. entry can exist in _EXACT and still be
    # displayed in English until every generated JSON file is rebuilt.
    for candidate in locale_fallback_chain(lang):
        if candidate == "en" and lang != "en":
            continue
        translated = load_locale_catalog(candidate).get(source)
        if translated:
            return translated
    ...
    # Exact application translations are also valid for dynamic messages that
    # are not yet materialized in every generated locale JSON.  Walk the same
    # regional fallback chain so es-419 (and other regional locales) does not
    # unexpectedly show the English source string.
    exact = _EXACT.get(source)
    if exact:
        for candidate in locale_fallback_chain(lang):
            translated = exact.get(candidate)
            if translated and (candidate == lang or candidate != "en"):
                return translated
    english_fallback = load_locale_catalog("en").get(source)
    if english_fallback:
        return english_fallback
    if lang == "en":
        return source
    legacy = _legacy_exact(source, lang)
    if legacy != source:
        return legacy
    if lang == "pl":
        return polish_translation(source)
    return source
...
_HISTORY_DETAIL_CANONICAL = {
    "menu": "menu",
    "Monitor conservador activo": "Conservative monitoring active",
    "Monitor apagado": "Monitoring stopped",
    "CPU / SMU operation completed": "CPU / SMU operation completed",
    "The privileged sync/drop_caches workflow completed successfully.": "The privileged sync/drop_caches workflow completed successfully.",
    "Opened terminal to configure nct6683 read-only monitoring.": "Opened terminal to configure nct6683 read-only monitoring.",
    "Opened terminal to prepare nct6687 PWM driver.": "Opened terminal to prepare nct6687 PWM driver.",
    "Opened terminal to disable nct6687 PWM preference.": "Opened terminal to disable nct6687 PWM preference.",
    "Lectura de cu_map.sh completada": "cu_map.sh reading completed",
    "Live-manager dashboard reading completed": "Live-manager dashboard reading completed",
    "Se abrió terminal para preparar dependencias faltantes.": "Opened a terminal to prepare missing dependencies.",
    "Se abrió terminal para preparar bc250_smu_oc/bc250-detect sin pip.": "Opened a terminal to prepare bc250_smu_oc/bc250-detect without pip.",
    "Se abrió terminal para instalar UMR.": "Opened a terminal to install UMR.",
    "Instalacion de servicio bc250-smu-oc solicitada": "bc250-smu-oc service installation requested",
    "Desactivacion de servicio bc250-smu-oc solicitada": "bc250-smu-oc service deactivation requested",
    "sigterm_conservador": "conservative SIGTERM",
    "sin_cierres": "no processes closed",
    "sugerir_cierre": "closing applications suggested",
    "ninguna": "none",
}

_HISTORY_FORMAT_SOURCES = (
    "PWM {0} set to {1}% ({2}/255).",
    "PWM {0} set to {1}%.",
    "Verified target: {0} / {1} CUs.",
    "GPU {0} °C → PWM {1} at {2}%",
    "GPU range updated to {0}–{1} MHz.",
)

HISTORY_DYNAMIC_SOURCES = tuple(dict.fromkeys(
    (*_HISTORY_TITLES.values(), *_HISTORY_DETAIL_CANONICAL.values(), *_HISTORY_FORMAT_SOURCES)
))

# Literal copy found by the full widget AST audit. Keeping this small list in
...
ADDITIONAL_VISIBLE_SOURCES = (
    ...
    "BC250 UMR GC namespace",
    "Bazzite memory setup",
    "Component",
    "Current session",
    "Choose the GPU write backend and telemetry fixes independently. Use kernel with both fixes disabled only on a kernel that already provides the BC250 range and metrics corrections.",
    "Cyan compatibility",
    "Cyan kernel compatibility",
    "Default (remove TTM limit)",
    "Enable or update table",
    "Enable fix-freq",
    "Enable fix-metrics",
    "NCT hardware monitor",
    "Kernel backend · patched BC250 kernels (Bazzite OGC / MastaG)",
    "No supported persistence backend",
    "Open kernel installation terminal",
    "Package trust",
    "Run the health check to inspect this BC250 installation.",
    "Review compatibility settings",
    "SMU direct backend · legacy fallback (max 1850 MHz)",
    "Driver inventory is loaded when this dialog is opened from the application.",
    "Kernel backend required above 1850 MHz",
    "The direct SMU backend is limited to 1850 MHz because loaded transitions to 2000 MHz can freeze the display or crash a game. Select the kernel backend on a compatible patched BC250 kernel, then retry.",
    "Saved to",
    "Stock CachyOS kernel remains installed",
    ...
    "This is not a standard dependency. It adds MastaG's external pacman repository, whose packages are unsigned (Optional TrustAll), then installs the BC-250 kernel and matching headers. The current stock CachyOS kernel is kept as a boot fallback. Continue only if you trust that upstream repository.",
    "Unsigned external repository (Optional TrustAll)",
...
def _translate_history_detail(detail: str, language: str) -> str:
    ...
    if pwm:
        ...
        templates = {
            ...
            "pl": "PWM {0} ustawiono na {1}% ({2}/255).",
        }
        template = templates.get(language) or tr(_HISTORY_FORMAT_SOURCES[0], language)
        return template.format(channel, percent, raw)
    ...
    if pwm_short:
        ...
        templates = {
            ...
            "pl": "PWM {0} ustawiono na {1}%.",
        }
        template = templates.get(language) or tr(_HISTORY_FORMAT_SOURCES[1], language)
        return template.format(channel, percent)
    ...
    if verified:
        ...
        templates = {
            ...
            "pl": "Zweryfikowany cel: {0} / {1} CU.",
        }
        template = templates.get(language) or tr(_HISTORY_FORMAT_SOURCES[2], language)
        return template.format(selected, total)
    ...
    if curve:
        ...
        templates = {
            ...
            "pl": "GPU {0} °C → PWM {1} przy {2}%",
        }
        template = templates.get(language) or tr(_HISTORY_FORMAT_SOURCES[3], language)
        return template.format(temperature, channel, percent)
    ...
    if range_match:
        ...
        templates = {
            ...
            "pl": "Zakres GPU zaktualizowano do {0}–{1} MHz.",
        }
        template = templates.get(language) or tr(_HISTORY_FORMAT_SOURCES[4], language)
        return template.format(minimum, maximum)
    ...
    polish_details = {
        "menu": "menu",
        "Monitor conservador activo": "Aktywny monitoring zachowawczy",
        "Monitor apagado": "Monitoring wyłączony",
        "CPU / SMU operation completed": "Operacja CPU / SMU zakończona",
        "The privileged sync/drop_caches workflow completed successfully.": "Uprzywilejowana procedura sync/drop_caches zakończyła się pomyślnie.",
        "Opened terminal to configure nct6683 read-only monitoring.": "Otwarto terminal, aby skonfigurować monitoring nct6683 tylko do odczytu.",
        "Opened terminal to prepare nct6687 PWM driver.": "Otwarto terminal, aby przygotować sterownik PWM nct6687.",
        "Opened terminal to disable nct6687 PWM preference.": "Otwarto terminal, aby wyłączyć preferencję PWM nct6687.",
        "Lectura de cu_map.sh completada": "Odczyt cu_map.sh zakończony",
        "Live-manager dashboard reading completed": "Odczyt panelu Live Manager zakończony",
        "Se abrió terminal para preparar dependencias faltantes.": "Otwarto terminal, aby przygotować brakujące zależności.",
        "Se abrió terminal para preparar bc250_smu_oc/bc250-detect sin pip.": "Otwarto terminal, aby przygotować bc250_smu_oc/bc250-detect bez pip.",
        "Se abrió terminal para instalar UMR.": "Otwarto terminal, aby zainstalować UMR.",
        "Instalacion de servicio bc250-smu-oc solicitada": "Zażądano instalacji usługi bc250-smu-oc",
        "Desactivacion de servicio bc250-smu-oc solicitada": "Zażądano wyłączenia usługi bc250-smu-oc",
        "sigterm_conservador": "zachowawczy SIGTERM",
        "sin_cierres": "bez zamykania procesów",
        "sugerir_cierre": "zalecane zamknięcie aplikacji",
        "ninguna": "brak",
    }
    if language == "pl" and source in polish_details:
        return polish_details[source]
...
_COUNT_FORMS["point"]["pl"] = ("punkt", "punkty", "punktów")
_COUNT_FORMS["WGP pair"]["pl"] = ("para WGP", "pary WGP", "par WGP")
_COUNT_FORMS["routed WGP"]["pl"] = ("trasowany WGP", "trasowane WGP", "trasowanych WGP")
_COUNT_FORMS["row"]["pl"] = ("wiersz", "wiersze", "wierszy")
_COUNT_FORMS["process"]["pl"] = ("proces", "procesy", "procesów")
_COUNT_FORMS["application"]["pl"] = ("aplikacja", "aplikacje", "aplikacji")
_COUNT_FORMS["event"]["pl"] = ("zdarzenie", "zdarzenia", "zdarzeń")


def _plural_form_index(count: int, language: str) -> int:
    if language in {"ru", "uk", "pl"}:
        last_two = count % 100
        last = count % 10
        ...
        return 2
    if language == "cs":
        return 0 if count == 1 else 1 if 2 <= count <= 4 else 2
    if language == "ro":
        if count == 1:
            return 0
        return 1 if count == 0 or 2 <= count % 100 <= 19 else 2
    return 0 if count == 1 else 1


def _localized_count_label(number: int, unit: str, language: str) -> str:
    forms = _DYNAMIC_COUNT_SOURCE_FORMS.get(unit)
    if not forms:
        return f"{number} {unit}"
    index = _plural_form_index(abs(number), language)
    example = (1, 2, 5)[index]
    source = f"{example} {forms[index]}"
    translated = tr(source, language)
    replaced, substitutions = re.subn(
        rf"(?<!\d){example}(?!\d)", str(number), translated, count=1
    )
    return replaced if substitutions else f"{number} {translated}"
...
def count_label(count: object, unit: str, language: str | None = None) -> str:
    try:
        ...
        number = 0
    lang = resolve_language(language or _CURRENT_LANGUAGE)
    return _localized_count_label(number, unit, lang)
...
def project_overview(language: str | None = None) -> str:
    lang = resolve_language(language or _CURRENT_LANGUAGE)
    return load_locale_catalog(lang).get(
        PROJECT_OVERVIEW["en"],
        PROJECT_OVERVIEW.get(lang, PROJECT_OVERVIEW["en"]),
    )
...
def daemon_details(language: str | None = None) -> str:
    ...
    return load_locale_catalog(lang).get(
        DAEMON_DETAILS["en"],
        DAEMON_DETAILS.get(lang, DAEMON_DETAILS["en"]),
    )
...
def _localize_text_widget(widget, language: str) -> None:
    """Localize a label/button/group while preserving explicit source text."""

    if bool(widget.property("i18nLiteral")):
        return
    explicit_source = getattr(widget, "source_text", None)
    if not isinstance(explicit_source, str) or not explicit_source:
        _translated_source(widget, widget.text, widget.setText, "i18nSourceText", language)
        return
    try:
        widget.setText("" if bool(widget.property("collapsed")) else tr(explicit_source, language))
        widget.setProperty("i18nSourceText", explicit_source)
        widget.setProperty("i18nSourceTextLanguage", language)
    except (RuntimeError, TypeError):
        logger.debug("Could not cache an explicit translation source on %r", widget, exc_info=True)


def _localized_sources(widget, property_name: str, count: int, getter, language: str) -> list[str]:
    previous = widget.property(property_name)
    old_language = str(widget.property(property_name + "Language") or language)
    sources = []
    for index in range(count):
        current = getter(index)
        old = previous[index] if isinstance(previous, list) and index < len(previous) else None
        if old is not None and current in {old, tr(old, old_language)}:
            source = old
        else:
            source = canonical_source(current, old_language)
            if source == current:
                source = canonical_source(current, language)
        sources.append(source)
    widget.setProperty(property_name, sources)
    widget.setProperty(property_name + "Language", language)
    return sources


def _localize_combo(widget, language: str) -> None:
    from PyQt6.QtCore import QSignalBlocker

    sources = _localized_sources(widget, "i18nComboSources", widget.count(), widget.itemText, language)
    # Translating display copy must not dispatch configuration actions.
...
def _localize_tabs(widget, language: str) -> None:
    sources = _localized_sources(widget, "i18nTabSources", widget.count(), widget.tabText, language)
    for index, source in enumerate(sources):
        widget.setTabText(index, tr(source, language))


def _table_header_sources(widget, language: str) -> list[str]:
...
def _localize_table_headers(widget, language: str) -> None:
    for column, source in enumerate(_table_header_sources(widget, language)):
        item = widget.horizontalHeaderItem(column)
...
def localize_widget_tree(root, language: str | None = None) -> None:
    ...
    for widget in widgets:
        if isinstance(widget, (QLabel, QAbstractButton, QGroupBox)):
            ...
            _localize_text_widget(widget, lang)
        ...
        try:
            ...
            _translated_source(widget, widget.accessibleName, widget.setAccessibleName, "i18nSourceAccessibleName", lang)
        except (RuntimeError, TypeError):
            logger.debug("Could not update a widget tooltip during localization", exc_info=True)
        if isinstance(widget, QComboBox):
            _localize_combo(widget, lang)
        if isinstance(widget, QTabWidget):
            _localize_tabs(widget, lang)
        if isinstance(widget, QTableWidget):
            _localize_table_headers(widget, lang)
    try:
...
def translation_coverage(strings: Iterable[str], languages: Iterable[str] | None = None) -> dict[str, list[str]]:
    ...
    for source in strings:
        if not re.search(r"[A-Za-z]{3}", source):
            continue
        # Catalog membership is the coverage signal. Some correct translations
        # are spelled exactly like English (for example French/Dutch
        # "Microcode"), so string inequality would report false fallbacks.
        unresolved = [lang for lang in languages if source not in load_locale_catalog(lang)]
        if unresolved:
...
def strict_translation_coverage(strings: Iterable[str], languages: Iterable[str] | None = None) -> dict[str, list[str]]:
    ...
    for source in strings:
        ...
        for language in languages:
            if source not in load_locale_catalog(language):
