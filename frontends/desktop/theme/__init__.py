...
def _finish_palette(palette: dict[str, str], mode: str) -> dict[str, str]:
    ...
    palette["cyan_hover"] = _blend(
        palette["cyan"], "#FFFFFF" if mode == "dark" else "#000000", 0.10
    )
...
def scale_stylesheet(stylesheet: str, scale: int | None = None) -> str:
    ...
    def replace(match: re.Match[str]) -> str:
        ...
        # Improve legibility without inflating card margins or spacing. Small
        # body/caption fonts gain one logical pixel while titles retain their
        # existing hierarchy; the user scale still applies on top.
        if value <= 13 and re.search(r"font-size\s*:\s*$", prefix):
            scaled += factor
        rendered = str(round(scaled)) if scaled >= 1 else f"{scaled:.1f}"
        return rendered + "px"
...
def application_stylesheet(mode: str | None = None, accent: str | None = None, density: str | None = None, scale: int | None = None) -> str:
    ...
    * {{
    ...
    QLabel[pill='true'][prominentPill='true'] {{
        padding: 5px 10px;
        font-size: 12px;
    }}
    ...
    QPushButton[dependencySectionTab='true']:hover {{
        background: {c['control_hover']};
        border-color: {c['blue_border']};
        color: {c['text']};
    }}
    QPushButton[dependencySectionTab='true']:checked {{
        background: {c['panel_raised']};
        border-color: {c['border_strong']};
        color: {c['text']};
        font-weight: 840;
    }}
    QFrame[dependencySystemBar='true'] {{
        background: {c['panel_alt']};
        border: 1px solid {c['border_soft']};
        border-radius: 10px;
    }}
    QLabel[dependencySystemText='true'] {{
        color: {c['text']};
        font-size: 12px;
        font-weight: 680;
    }}
    ...
    QFrame[dashboardHero='true'],
    QFrame[dashboardModule='true'],
    QFrame[dashboardPreparation='true'] {{
        background: {c['panel']};
        border: 1px solid {c['border']};
        border-radius: 14px;
    }}
    QFrame[dashboardEvidence='true'] {{
        background: {c['panel']};
        border: none;
    ...
    QLabel[dashboardCardTitle='true'] {{
        color: {c['text']};
        font-size: 13px;
        font-weight: 750;
    }}
    QLabel[dashboardCardSubtitle='true'] {{
        color: {c['muted']};
        font-size: 10px;
    }}
    QFrame[dashboardMetricTile='true'] {{
        background: {c['panel_alt']};
        border: 1px solid {c['border_soft']};
        border-radius: 10px;
    }}
    QFrame[dashboardMetricTile='true'][tone='blue'] {{
        border-color: {c['blue_border']};
    }}
    QFrame[dashboardMetricTile='true'][tone='orange'] {{
        border-color: {c['orange_border']};
    }}
    QLabel[dashboardMetricLabel='true'] {{
        color: {c['muted']};
        font-size: 10px;
        font-weight: 650;
    }}
    QLabel[dashboardMetricValue='true'] {{
        color: {c['text']};
        font-size: 15px;
        font-weight: 750;
    }}
    QLabel[dashboardMetricDetail='true'] {{
        color: {c['muted']};
        font-size: 9px;
    }}
    ...
    QLabel[dashboardFrequencyValue='true'] {{
        color: {c['text']};
        font-size: 64px;
        font-weight: 800;
    }}
    QLabel[dashboardFrequencyUnit='true'] {{
        color: {c['muted']};
        font-size: 18px;
        font-weight: 780;
        padding-bottom: 9px;
    }}
    QLabel[dashboardFrequencyEyebrow='true'] {{
        color: {c['muted']};
        font-size: 10px;
        font-weight: 820;
    }}
    QLabel[dashboardFrequencyCopy='true'] {{
        color: {c['muted']};
        font-size: 10px;
    }}
    QLabel[dashboardEvidenceLabel='true'] {{
        color: {c['muted']};
        font-size: 10px;
    }}
    QLabel[dashboardEvidenceValue='true'] {{
        color: {c['text']};
        font-size: 10px;
        font-weight: 760;
    }}
    QPushButton[dashboardPrimaryAction='true'] {{
        min-height: 22px;
        padding: 8px 12px;
        background: {c['blue']};
    ...
    QLabel[dashboardModuleHeadline='true'] {{
        color: {c['text']};
        font-size: 26px;
        font-weight: 780;
    }}
    QLabel[dashboardModuleUnit='true'] {{
        color: {c['muted']};
        font-size: 13px;
        font-weight: 730;
        padding-bottom: 4px;
    }}
    QPushButton[dashboardCardAction='true'] {{
        min-height: 20px;
        padding: 6px 10px;
        background: {c['panel_alt']};
        border: 1px solid {c['border_soft']};
    ...
    QPushButton[dashboardCardAction='true']:hover,
    QPushButton[dashboardCardAction='true']:focus {{
        background: {c['control_hover']};
        border-color: {c['blue_border']};
        color: {c['blue']};
    }}
    ...
    QFrame[dashboardPreparationSystem='true'] {{
        background: {c['panel_alt']};
        border-top: 1px solid {c['border_soft']};
        border-bottom: 1px solid {c['border_soft']};
    }}
    QPushButton[dashboardPreparationTab='true'] {{
        min-height: 22px;
        padding: 5px 10px;
        background: transparent;
    ...
    QPushButton[dashboardPreparationTab='true']:checked {{
        background: {c['blue_soft']};
        color: {c['blue']};
    }}
    QPushButton[dashboardPreparationTab='true']:hover,
    QPushButton[dashboardPreparationTab='true']:focus {{
        border-color: {c['border_strong']};
        color: {c['text']};
    }}
    QFrame[dashboardComponentCard='true'],
    QFrame[dashboardPreparationInfo='true'] {{
        background: {c['panel_alt']};
        border: 1px solid {c['border_soft']};
        border-radius: 11px;
    }}
    ...
    QFrame[dashboardCompatibilityActions='true'],
    QFrame[dashboardCompatibilityState='true'] {{
        background: {c['panel']};
        border: 1px solid {c['border_soft']};
        border-radius: 9px;
        padding: 6px;
    ...
    QLabel[dashboardComponentTitle='true'] {{
        color: {c['text']};
        font-size: 12px;
        font-weight: 650;
    }}
    QLabel[dashboardComponentDetail='true'] {{
        color: {c['muted']};
        font-size: 9px;
    }}
    QLabel[dashboardDeckyPreview='true'] {{
        background: {c['panel']};
        border: 1px solid {c['border_soft']};
        border-radius: 10px;
        padding: 8px;
    }}
    QPushButton[dashboardComponentCheck='true'] {{
        width: 26px;
        height: 26px;
        min-width: 26px;
        max-width: 26px;
    ...
    QPushButton[dashboardComponentCheck='true']:checked {{
        background: {c['panel_raised']};
        border-color: {c['border_strong']};
        color: {c['text']};
    }}
    QFrame[dashboardMemoryNote='true'],
    QFrame[dashboardMemoryPanel='true'] {{
        background: {c['panel_alt']};
        border: 1px solid {c['border_soft']};
        border-radius: 8px;
    }}
    ...
    QPushButton[dependencyPrepareButton='true'][dashboardPrepareAction='true'] {{
        min-height: 22px;
        padding: 7px 16px;
        font-size: 12px;
        font-weight: 700;
    }}
    QPushButton[dashboardFooterAction='true'] {{
        min-width: 34px;
        max-width: 34px;
        min-height: 34px;
        max-height: 34px;
    ...
    QPushButton[dashboardFooterAction='true']:hover,
    QPushButton[dashboardFooterAction='true']:focus {{
        background: {c['blue_soft']};
        border-color: {c['blue_border']};
        color: {c['blue']};
    }}
    QPushButton[dashboardFooterAction='true'][donation='true']:hover,
    QPushButton[dashboardFooterAction='true'][donation='true']:focus {{
        background: {c['control_hover']};
        border-color: {c['border_strong']};
    }}
    ...
    QPushButton[dashboardGhostAction='true'] {{
        min-height: 22px;
        padding: 7px 10px;
        background: transparent;
        border: 1px solid {c['border']};
    ...
    QFrame[dependencyActionTile='true'][dependencyLeadTile='true'] {{
        background: {c['panel_raised']};
        border-color: {c['border']};
    }}
    QFrame[metricTile='true'][dependencyGovernorSelected='true'] {{
        background: {c['panel_raised']};
        border-color: {c['blue_border']};
    }}
    QPushButton[dependencyGovernorAction='true'] {{
        min-height: 18px;
        max-height: 18px;
        padding: 7px 11px;
        border-radius: 9px;
    ...
    QPushButton[dependencyGfxAction='true'] {{
        min-height: 20px;
        max-height: 20px;
        padding: 7px 9px;
        border-radius: 9px;
    ...
    QPushButton[dependencyGfxAction='true']:hover {{
        background: {c['control_hover']};
        border-color: {c['border_strong']};
        color: {c['text']};
    }}
    ...
    QFrame[compactSafetyNotice='true'] QLabel[noticeTitle='true'][prominentNoticeTitle='true'] {{
        font-size: 12px;
        font-weight: 800;
    }}
    QFrame[compactSafetyNotice='true'] QLabel[noticeBody='true'][prominentNoticeBody='true'] {{
        font-size: 11px;
    }}
