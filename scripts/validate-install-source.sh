...
PROJECT_ROOT="${1:-$(dirname "$(dirname "$SCRIPT_PATH")")}" 
MODE="${2:-full}"
...
required=(
  ...
  scripts/bc250-gpu-voltage-lab.sh
  scripts/bc250-control-center
  scripts/bc250-control-center-cli
  scripts/bc250-control-centerd
...
for relative in \
  scripts/install-local.sh scripts/uninstall-local.sh scripts/update-local.sh \
  scripts/install-decky-quick-access.sh \
  scripts/validate-install-source.sh scripts/smoke-install-layout.sh \
  scripts/steamos-root.sh \
