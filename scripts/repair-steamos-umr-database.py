...
def _chmod_tree(target: Path, owner_uid: int | None = None) -> None:
    for root, dirs, files in os.walk(target):
        try:
            # Shared read-only database directory must be traversable by the service.
            os.chmod(root, 0o755)  # nosec B103
...
def _install_tree_atomic(source: Path, target: Path, *, move_source: bool, owner_uid: int | None = None) -> tuple[dict[str, str], bool]:
    ...
    try:
        ...
        # Normalize only the isolated staging tree. Once it becomes visible at
        # target, the complete payload has already passed every fallible check.
        _chmod_tree(staging, owner_uid)
        ...
        if not swapped and old.exists() and not target.exists():
            old.rename(target)
...
def _parse_args(argv=None):
    ...
    parser.add_argument(
        "--print-selector",
        action="store_true",
        help="validate the current database and print only its cyan_skillfish register selector",
    )
...
def _inspect_current(target: Path) -> tuple[dict[str, str] | None, str]:
    try:
        return validate_database(target, require_pci_mapping=True), ""
    except DatabaseError as exc:
        return None, str(exc)


def _cleanup_legacy_if_requested(args, legacy_root: Path) -> None:
    if not args.cleanup_legacy:
        return
...
def _finish_valid_current(args, target: Path, legacy_root: Path, current: dict[str, str]) -> int:
    ...
    if args.update_service_config and update_service_config(
        args.service_config, target, current["selector"]
    ):
        print(f"[OK] Updated service UMR path: {args.service_config}")
    _cleanup_legacy_if_requested(args, legacy_root)
    return 0


def _prepare_target_parent(target: Path, owner_uid: int | None) -> None:
    target.parent.mkdir(parents=True, exist_ok=True)
    if owner_uid is None:
        return
    try:
        os.chown(target.parent, owner_uid, -1)
        # The unprivileged service must be able to traverse this parent.
        os.chmod(target.parent, 0o755)  # nosec B103
    except OSError:
        pass


def _legacy_database_details(legacy_root: Path, target: Path):
    legacy_database = legacy_root / "database"
    if legacy_database.resolve() == target:
        return legacy_database, None
    try:
        details = validate_database(legacy_database, require_pci_mapping=False)
    except DatabaseError:
        details = None
    return legacy_database, details


def _install_repaired_database(args, target: Path, legacy_root: Path):
    legacy_database, legacy_details = _legacy_database_details(legacy_root, target)
    if legacy_details is not None:
        print(
            "[INFO] Migrating the valid legacy database out of the small /var partition: "
            f"{legacy_database}"
        )
        installed, mapping_added = _install_tree_atomic(
            legacy_database,
            target,
            move_source=False,
            owner_uid=args.owner_uid,
        )
        return installed, mapping_added, legacy_details

    # Put download and staging on the target filesystem so final publication
    # stays atomic and the small SteamOS /var partition is never filled.
    with tempfile.TemporaryDirectory(
        prefix=".bc250-umr-download-", dir=target.parent
    ) as tmp:
    ...
    return installed, mapping_added, source_details


def _print_repair_result(
    args,
    target: Path,
    ...
    if args.update_service_config and update_service_config(
        args.service_config, target, installed["selector"]
    ):
        print(f"[OK] Updated service UMR path: {args.service_config}")
...
def main(argv=None) -> int:
    ...
    current, current_error = _inspect_current(target)

    if current is not None and not args.force:
        return _finish_valid_current(args, target, legacy_root, current)

    if args.check_only or args.print_selector:
        print(f"[ERR] SteamOS UMR database is invalid: {current_error}", file=sys.stderr)
        return 42

    _prepare_target_parent(target, args.owner_uid)
    ...
    try:
        installed, mapping_added, source_details = _install_repaired_database(
            args, target, legacy_root
        )
    except OSError as exc:
    ...
    _print_repair_result(args, target, installed, mapping_added, source_details)
    _cleanup_legacy_if_requested(args, legacy_root)
    return 0
