...
from bc250cc.infrastructure.persistence.config_paths import (
    legacy_qt_config_dir,
    ui_settings_path,
)
...
class UiPreferences:
    ...
    def _migrate_legacy_safety_preferences(self) -> None:
        ...
        # The notifications UI is hidden and its runtime is fail-closed.  Also
        # migrate any prior opt-in values back to disabled on startup.
        ...
        # The keypad is now one coherent feature: when enabled for a gamepad it
        # also opens automatically on editable fields. Keep the legacy key in
        # sync so older builds can still read the same preference.
        keypad_enabled = str(
            self.settings.value("settings/gamepad_onscreen_keypad", "true")
        ).strip().lower() in {"1", "true", "yes", "on"}
        self.settings.setValue(
            "settings/gamepad_keypad_auto_show",
            "true" if keypad_enabled else "false",
        )
